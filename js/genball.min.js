...
genball.generators.game = function(data, kickdata, home, away, gameId, startTime, gameSeed, week, debug) {
    ...
    var maxStatUpdate = function(player, team, stat, change) {
        ...
        player.seasonStats[stat] = _.max([ player.stats[gameId][stat] || 0, change ]);
        team.stats[gameId][stat] = _.max([ player.stats[gameId][stat] || 0, change ]);
        team.seasonStats[stat] = _.max([ player.stats[gameId][stat] || 0, change ]);
    };
...
genball.generators.play = function(data) {
    var getDifferenital = function(name, teams, scoreboard) {
        if (name === teams.home.name) {
            return scoreboard.homeScores.total() - scoreboard.awayScores.total();
        }
        return scoreboard.awayScores.total() - scoreboard.homeScores.total();
    };
    ...
    var nextPlay = function(quarter, down, distance, at, possession, teams, scoreboard, clock) {
        var possible = [];
        var differential = getDifferenital(possession, teams, scoreboard);
        var avoidTypes = [];
        ...
        } else if (quarter === 4 && differential > -4 && differential < 0 && clock < 25 && at < 50) {
