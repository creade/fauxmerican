...
genball.generators.game = function(data, kickdata, home, away, gameId, startTime, debug) {
    var playGenerator = genball.generators.play(data);
    var kickoffGenerator = genball.generators.kickoff(kickdata);
    var scoreboard = genball.models.scoreboard().scoreboard(home.name, away.name);
    var teams = {};
    var completed = false;
    ...
    function downFormatter(down) {
        var downs = [ "1st", "2nd", "3rd", "4th" ];
        return downs[down - 1];
    }
    function atFormatter(driveState) {
        var at = "";
        if (driveState.at <= 50) {
            if (driveState.possession === home.name) {
                at += away.shortName;
    ...
    function extraPoint(driveState) {
        var team = teams[driveState.possession];
        var kicker = team.getKicker();
        kicker.stats[gameId]["XPA"] = (kicker.stats[gameId]["XPA"] || 0) + 1;
        kicker.seasonStats["XPA"] = (kicker.seasonStats["XPA"] || 0) + 1;
        var chance = Math.random();
        driveState.commentary += kicker.fullName;
        if (chance <= kicker.skill) {
            driveState.commentary += " extra point GOOD.";
            kicker.stats[gameId]["XPM"] = (kicker.stats[gameId]["XPM"] || 0) + 1;
            kicker.seasonStats["XPM"] = (kicker.seasonStats["XPM"] || 0) + 1;
    ...
    function kickoffSetup(driveState) {
    ...
    function clockFormatter(driveState) {
        var pad = function(n) {
            return n < 10 ? "0" + n : n;
        };
    ...
    function incompletePass(driveState, play) {
        if (isTurnoverOnDowns(play, driveState)) {
            return passingTurnoverOnDowns(driveState, play);
        }
        var team = teams[driveState.possession];
        var passer = team.getPasser();
        passer.stats[gameId]["PA"] = (passer.stats[gameId]["PA"] || 0) + 1;
        passer.stats[gameId]["C"] = passer.stats[gameId]["C"] || 0;
        passer.seasonStats["PA"] = (passer.seasonStats["PA"] || 0) + 1;
        passer.seasonStats["C"] = passer.seasonStats["C"] || 0;
        driveState.down++;
        driveState.commentary = passer.fullName + " PASS INCOMPLETE.";
        return driveState;
    }
    function sack(driveState, play) {
        if (play.safety) {
            return passingSafety(driveState, play);
        }
        var team = teams[driveState.possession];
        var passer = team.getPasser();
        passer.stats[gameId]["RA"] = (passer.stats[gameId]["RA"] || 0) + 1;
        passer.stats[gameId]["RYDS"] = (passer.stats[gameId]["RYDS"] || 0) + play.yards;
        passer.seasonStats["RA"] = (passer.seasonStats["RA"] || 0) + 1;
        passer.seasonStats["RYDS"] = (passer.seasonStats["RYDS"] || 0) + play.yards;
        driveState.commentary = "";
        if (play.recoveredFumble) {
            passer.stats[gameId]["FU"] = (passer.stats[gameId]["FU"] || 0) + 1;
            passer.seasonStats["FU"] = (passer.seasonStats["FU"] || 0) + 1;
    ...
    function pass(driveState, play) {
        ...
        passer.stats[gameId]["PA"] = (passer.stats[gameId]["PA"] || 0) + 1;
        passer.stats[gameId]["C"] = (passer.stats[gameId]["C"] || 0) + 1;
        passer.stats[gameId]["PY"] = (passer.stats[gameId]["PY"] || 0) + play.yards;
        receiver.stats[gameId]["R"] = (receiver.stats[gameId]["R"] || 0) + 1;
        receiver.stats[gameId]["RECY"] = (receiver.stats[gameId]["RECY"] || 0) + play.yards;
        passer.seasonStats["PA"] = (passer.seasonStats["PA"] || 0) + 1;
        passer.seasonStats["C"] = (passer.seasonStats["C"] || 0) + 1;
        passer.seasonStats["PY"] = (passer.seasonStats["PY"] || 0) + play.yards;
        receiver.seasonStats["R"] = (receiver.seasonStats["R"] || 0) + 1;
        receiver.seasonStats["RECY"] = (receiver.seasonStats["RECY"] || 0) + play.yards;
    ...
    function passingSafety(driveState, play) {
        var team = teams[driveState.possession];
        var passer = team.getPasser();
        passer.stats[gameId]["RA"] = (passer.stats[gameId]["RA"] || 0) + 1;
        passer.stats[gameId]["RYDS"] = (passer.stats[gameId]["RYDS"] || 0) + play.yards;
    ...
    function rushingSafety(driveState, play) {
        var team = teams[driveState.possession];
        var rusher = team.getRusher();
        rusher.stats[gameId]["RA"] = (rusher.stats[gameId]["RA"] || 0) + 1;
        rusher.stats[gameId]["RYDS"] = (rusher.stats[gameId]["RYDS"] || 0) + play.yards;
        rusher.seasonStats["RA"] = (rusher.seasonStats["RA"] || 0) + 1;
        rusher.seasonStats["RYDS"] = (rusher.seasonStats["RYDS"] || 0) + play.yards;
        driveState.commentary = "";
        if (play.recoveredFumble) {
            rusher.stats[gameId]["FU"] = (rusher.stats[gameId]["FU"] || 0) + 1;
            rusher.seasonStats["FU"] = (rusher.seasonStats["FU"] || 0) + 1;
    ...
    function passingTouchdown(driveState, play) {
        var team = teams[driveState.possession];
        var passer = team.getPasser();
        var receiver = team.getReceiver();
        passer.stats[gameId]["PA"] = (passer.stats[gameId]["PA"] || 0) + 1;
        ...
        receiver.stats[gameId]["R"] = (receiver.stats[gameId]["R"] || 0) + 1;
        receiver.stats[gameId]["RECY"] = (receiver.stats[gameId]["RECY"] || 0) + play.yards;
        passer.stats[gameId]["PTD"] = (passer.stats[gameId]["PTD"] || 0) + 1;
        receiver.stats[gameId]["RECTD"] = (receiver.stats[gameId]["RECTD"] || 0) + 1;
        passer.seasonStats["PA"] = (passer.seasonStats["PA"] || 0) + 1;
        passer.seasonStats["C"] = (passer.seasonStats["C"] || 0) + 1;
        passer.seasonStats["PY"] = (passer.seasonStats["PY"] || 0) + play.yards;
        receiver.seasonStats["R"] = (receiver.seasonStats["R"] || 0) + 1;
        receiver.seasonStats["RECY"] = (receiver.seasonStats["RECY"] || 0) + play.yards;
        passer.seasonStats["PTD"] = (passer.seasonStats["PTD"] || 0) + 1;
        receiver.seasonStats["RECTD"] = (receiver.seasonStats["RECTD"] || 0) + 1;
    ...
    function rushingTouchdown(driveState, play) {
        var team = teams[driveState.possession];
        var rusher = team.getRusher();
        if (play.recoveredFumble) {
            rusher.stats[gameId]["FU"] = (rusher.stats[gameId]["FU"] || 0) + 1;
        ...
        rusher.stats[gameId]["RA"] = (rusher.stats[gameId]["RA"] || 0) + 1;
        rusher.stats[gameId]["RYDS"] = (rusher.stats[gameId]["RYDS"] || 0) + play.yards;
        rusher.stats[gameId]["RTDS"] = (rusher.stats[gameId]["RTDS"] || 0) + 1;
        rusher.seasonStats["RA"] = (rusher.seasonStats["RA"] || 0) + 1;
        rusher.seasonStats["RYDS"] = (rusher.seasonStats["RYDS"] || 0) + play.yards;
        rusher.seasonStats["RTDS"] = (rusher.seasonStats["RTDS"] || 0) + 1;
        driveState.commentary = rusher.fullName + " " + play.yards + " yd RUSHING TOUCHDOWN. ";
        driveState = extraPoint(driveState);
        return kickoffSetup(driveState);
    }
    function passingTurnoverOnDowns(driveState, play) {
        var team = teams[driveState.possession];
        var passer = team.getPasser();
        if (play.sack) {
            passer.stats[gameId]["RA"] = (passer.stats[gameId]["RA"] || 0) + 1;
    ...
    function rushingTurnoverOnDowns(driveState, play) {
        var team = teams[driveState.possession];
        var rusher = team.getRusher();
        rusher.stats[gameId]["RA"] = (rusher.stats[gameId]["RA"] || 0) + 1;
        rusher.stats[gameId]["RYDS"] = (rusher.stats[gameId]["RYDS"] || 0) + play.yards;
    ...
    function fumble(driveState, play) {
        var team = teams[driveState.possession];
        var player = _.pickRandom([ team.getRusher(), team.getPasser() ]);
        var recoverer = teams[otherTeam(driveState)].getRecoverer();
        player.stats[gameId]["FU"] = (player.stats[gameId]["FU"] || 0) + 1;
        player.seasonStats["FU"] = (player.seasonStats["FU"] || 0) + 1;
        player.stats[gameId]["FL"] = (player.stats[gameId]["FL"] || 0) + 1;
        player.seasonStats["FL"] = (player.seasonStats["FL"] || 0) + 1;
        recoverer.stats[gameId]["FR"] = (recoverer.stats[gameId]["FR"] || 0) + 1;
        recoverer.seasonStats["FR"] = (recoverer.seasonStats["FR"] || 0) + 1;
    ...
    function interception(driveState, play) {
        var team = teams[driveState.possession];
        var passer = team.getPasser();
        passer.stats[gameId]["PA"] = (passer.stats[gameId]["PA"] || 0) + 1;
        passer.stats[gameId]["C"] = passer.stats[gameId]["C"] || 0;
        passer.stats[gameId]["INT"] = (passer.stats[gameId]["INT"] || 0) + 1;
        passer.seasonStats["PA"] = (passer.seasonStats["PA"] || 0) + 1;
        passer.seasonStats["C"] = passer.seasonStats["C"] || 0;
        passer.seasonStats["INT"] = (passer.seasonStats["INT"] || 0) + 1;
        driveState.commentary = passer.fullName + " INTERCEPTED. ";
        driveState.at = driveState.at - play.yards;
        var driveState = changePossession(driveState);
        return driveState;
    }
    function fieldGoal(play, driveState) {
        var team = teams[driveState.possession];
        var kicker = team.getKicker();
        kicker.stats[gameId]["FGA"] = (kicker.stats[gameId]["FGA"] || 0) + 1;
        kicker.seasonStats["FGA"] = (kicker.seasonStats["FGA"] || 0) + 1;
        var updatedState;
        driveState.commentary = kicker.fullName + " " + (driveState.at + 18);
        if (play.fieldGoalMade) {
            kicker.stats[gameId]["FGM"] = (kicker.stats[gameId]["FGM"] || 0) + 1;
            kicker.seasonStats["FGM"] = (kicker.seasonStats["FGM"] || 0) + 1;
            kicker.stats[gameId]["FGL"] = _.max([ kicker.stats[gameId]["FGL"] || 0, driveState.at + 18 ]);
            kicker.seasonStats["FGL"] = _.max([ kicker.seasonStats["FGL"] || 0, driveState.at + 18 ]);
            ...
            kicker.stats[gameId]["FGM"] = kicker.stats[gameId]["FGM"] || 0;
            kicker.seasonStats["FGM"] = kicker.seasonStats["FGM"] || 0;
            driveState.commentary += " yd FIELD GOAL NO GOOD";
            updatedState = changePossession(driveState);
        }
        return updatedState;
    }
    function punt(driveState, play) {
        var team = teams[driveState.possession];
        var punter = team.getPunter();
        punter.stats[gameId]["P"] = (punter.stats[gameId]["P"] || 0) + 1;
        punter.seasonStats["P"] = (punter.seasonStats["P"] || 0) + 1;
        var updatedDriveState;
        driveState.commentary = punter.fullName + " " + play.distance + " yd PUNT";
        punter.stats[gameId]["PL"] = _.max([ punter.stats[gameId]["PL"] || 0, play.distance ]);
        punter.seasonStats["PL"] = _.max([ punter.seasonStats["PL"] || 0, play.distance ]);
        if (play.fairCatch) {
            ...
            if (play.touchdown) {
                driveState.at = driveState.at - play.distance;
                driveState = changePossession(driveState);
                driveState.commentary += " to the " + atFormatter(driveState) + ". RETURNED BY " + driveState.possession + " " + play.returnDistance + " yds for a TOUCHDOWN. ";
                driveState = extraPoint(driveState);
                ...
                driveState = changePossession(driveState);
                driveState.distance = Math.min(10, driveState.at);
                driveState.commentary += " to the " + atFormatter(driveState) + ". RETURNED BY " + driveState.possession + " " + play.returnDistance + " yds";
    ...
    function kickoff(driveState, play) {
        var team = teams[driveState.possession];
        var kicker = team.getKicker();
        kicker.stats[gameId]["KO"] = (kicker.stats[gameId]["KO"] || 0) + 1;
        kicker.seasonStats["KO"] = (kicker.seasonStats["KO"] || 0) + 1;
        var updatedDriveState;
    ...
    updateDriveState = function(play, driveState) {
    ...
    var playGame = function() {
        ...
        driveState.driveNumber++;
        driveState.possession = otherTeam(driveState);
        driveState = kickoffSetup(driveState);
        ...
        if (scoreboard.scores[home.name].total > scoreboard.scores[away.name].total) {
            home.record.wins++;
            away.record.losses++;
        } else {
            home.record.losses++;
    ...
    var playUntil = function(currentTime) {
        ...
        driveState.quarter = 3;
        driveState.clock = 900;
        if (driveState.time > currentTime && driveState.quarter === 3) {
            return {
                teams: teams,
                scoreboard: scoreboard,
                gameId: gameId,
        ...
        driveState.quarter = 4;
        driveState.clock = 900;
        if (driveState.time > currentTime && driveState.quarter === 4) {
            return {
                teams: teams,
                scoreboard: scoreboard,
                gameId: gameId,
    ...
    var play = function() {
        ...
        if (driveState.kickoff) {
            ...
            play = playGenerator.nextPlay(driveState.quarter, driveState.down, driveState.distance, driveState.at);
        }
...
genball.generators.play = function(data) {
    var nextPlay = function(quarter, down, distance, at) {
        var possible = _.select(data[quarter], function(play) {
        ...
        var play = _.pickRandom(possible);
        if (!play) {
            console.log('"Dn":' + down + ',"Dis":' + distance + ',"At":' + at);
            play = {
                F: 2,
                Q: 3,
                Dn: 2,
...
genball.models.punt = function(puntData) {
    return {
        ...
        returned: !!puntData.Ry,
        returnDistance: puntData.Ry,
        touchdown: puntData.Td,
        distance: puntData.Y
    };
};
...
genball.models.scoreboard = function() {
    var quarters = {
        1: "first",
        2: "second",
        3: "third",
    ...
    var teamScore = function(team) {
        return {
            scoringPlays: ko.observableArray(),
            first: ko.observable(0),
            second: ko.observable(0),
            third: ko.observable(0),
            fourth: ko.observable(0),
            total: ko.observable(0)
        };
    };
    var scoreboard = function(home, away) {
        var teamScores = {};
        homeScores = teamScore();
        awayScores = teamScore();
        var score = function(team, driveState, play, points, quarter) {
            ...
            line.scoringPlays.push(genball.models.driveState().clone(driveState, play, this));
            line.total(line.total() + points);
            line[quarters[quarter]](line[quarters[quarter]]() + points);
        };
        return {
            score: score,
...
genball.models.team = function(info, coach, OC, DC, tempo, players, institution, stadium, id, logo) {
    ...
    var getRecoverer = function() {
        return _.chain(players).select(function(player) {
    ...
    var generators = {
        ...
        logoGenerator: function(logos) {
            var logoFor = function(info, institution) {
                if (info.L) {
                    ...
                    var font = text.length <= 2 && Math.random() > .5 ? "Holtwood One SC" : "Graduate";
        ...
        teamGenerator: function(teamInfoGenerator, fullNameGenerator, institutionGenerator, playerGenerator, stadiumGenerator, uniformGeneratorGenerator, logoGenerator) {
            ...
            var next = function() {
                ...
                var id = teamId;
                teamId++;
                return genball.models.team(info, coach, OC, DC, tempo, players, institution, stadium, id, logo);
            };
            return {
                next: next
            };
