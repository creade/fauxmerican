...
genball.generators.game = function(data, kickdata, home, away, gameId, startTime, gameSeed, week, debug) {
    ...
    var passingTurnoverOnDowns = function(driveState, play) {
        ...
        driveSummaryEnd("TURNOVER ON DOWNS", driveState);
...
genball.generators.schedule = function() {
    ...
    var scheduleEventsConcurrent = function(startDay) {
        var events = [];
        events.push({
            startTime: moment.tz("America/Detroit").dayOfYear(startDay - 1).hour(0).minute(0).second(0),
            action: function(teams) {
                teams = _.sortBy(teams, "skill");
                var currentRank = 15;
                teams.reverse();
            ...
            events.push({
                ...
                action: function(teams) {
                    ...
                    _.chain(lossArray).map(function(lossNumber) {
                        ...
                        _.each(teams, function(team) {
                            ...
                            if (lastGame.winner() === team.name) {
                                ...
                                console.log("In week: " + index + " team: " + team.id + " won a game, moving them to  record: " + team.record.wins + " and " + team.record.losses + " changing their ranking from " + oldRanking + "  to " + team.ranking);
                            } else {
                                ...
                                existantRatings.push(potentialRating);
                                team.ranking = potentialRating;
                                console.log("In week: " + index + " team: " + team.id + " lost a game, moving them to  record: " + team.record.wins + " and " + team.record.losses + " changing their ranking from " + oldRanking + "  to " + team.ranking);
                            }
                        });
                    ...
                    _.each(teams, function(team) {
                        ...
                        var oldRanking = team.ranking;
                        if (lastGame.winner() === team.name) {
                            var potentialRating = _.max([ team.ranking - _.pickRandom([ 0, 1, 2 ]), 4 ]);
                            while (_.contains(existantRatings, potentialRating)) {
                                potentialRating++;
                            }
                            team.ranking = potentialRating;
